...
const MATERIALS = {
    ...
    granite: {
        ...
        basePrice: 799,
        type: 'Stone',
        texturePath: '/textures/marble_diffuse.jpg', // Reusing noise pattern
        variants: [
...
function App() {
    // --- State ---
    const [selectedCategory, setSelectedCategory] = useState('Wood') // 'Wood' or 'Stone'
    ...
    // Merge base props with variant specific color
    ...
    // Helper to change category and default to first material in that category
    ...
    return (
        <div className="flex w-full h-screen bg-white font-sans text-slate-800 overflow-hidden">

            {/* --- Left Panel: 3D Viewer --- */}
            <div className="relative w-full lg:w-2/3 h-full bg-slate-100">
                ...
                <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
                    <button className="flex items-center gap-2 bg-white/90 backdrop-blur px-6 py-3 rounded-full shadow-lg hover:shadow-xl transition-all border border-slate-200 text-sm font-semibold">
                ...
                <div className="absolute top-8 left-8">
                    <h2 className="text-xl font-bold tracking-tight text-slate-900">Tirth Joshi</h2>
                </div>
            </div>

            {/* --- Right Panel: Configurator Sidebar --- */}
            <div className="w-full lg:w-1/3 h-full overflow-y-auto bg-white border-l border-slate-200 flex flex-col">

                {/* Header (Sticky) */}
                <div className="p-8 pb-4 border-b border-slate-100 bg-white sticky top-0 z-20">
                    <div className="flex justify-between items-start mb-2">
                        <h1 className="text-2xl font-bold text-slate-900">3D Table Configurator</h1>
                        <div className="text-right">
                            <span className="block text-2xl font-bold text-indigo-600">€{totalPrice},-</span>
                            <span className="text-xs text-slate-400">Incl. VAT</span>
                        </div>
                    </div>
                    <p className="text-slate-500 text-sm">Design your custom table in 4 simple steps.</p>
                </div>
                ...
                <div className="flex-1 p-8 space-y-10">
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-semibold text-lg">Choose Material</h3>
                        </div>
                    ...
                    ${selectedMaterialKey === config.id ? 'border-indigo-600 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-300'}`}
                                    >
                                        <span className="relative z-10 font-medium">{config.name}</span>
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-semibold text-lg">Shape & Size</h3>
                        </div>
                    ...
                    ${selectedShape === shape.id
                        ...
                        <select
                            ...
                            className="w-full p-3 rounded-lg border border-slate-200 bg-slate-50 text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        >
                            {SIZES.map(s => (
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-semibold text-lg">Legs / Base</h3>
                        </div>
                ...
                <div className="p-8 border-t border-slate-100 bg-slate-50">
                    ...
                    <div className="mb-4 flex items-center gap-2 text-indigo-600 text-sm font-medium bg-indigo-50 p-2 rounded-lg">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        Delivered within 4-6 weeks
                    </div>

                    <button className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-black transition-all shadow-lg hover:shadow-xl active:scale-95">
